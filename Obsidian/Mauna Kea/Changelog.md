...
- **Wobble-Free Sidebar Accordion Navigation**: Resolved sidebar layout shifting by replacing unstable height hover calculations with rock-solid CSS Grid accordion expansion (`grid-template-rows: 1fr`/`0fr`) and explicit section state management in `Sidebar.tsx`. Hovering now provides instant gold highlight feedback without causing adjacent options to wobble or jump up and down.
