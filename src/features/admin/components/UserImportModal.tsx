...
import { mapUsersAction, checkUserDuplicatesAction, finalizeUserImportAction } from "@/actions/imports";
...
export default function UserImportModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [newUsersQueue, setNewUsersQueue] = useState<any[]>([]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data: any = await mapUsersAction(validHeaders, sampleData);
  ...
  const confirmImport = async () => {
    ...
    if (!importMapping["email"]) {
      toast.error("Email column must be mapped to import users.");
      return;
    }
    ...
    setIsImporting(true);
    try {
      const mappedUsers = importFileData.map((row) => {
        const u: any = {};
        Object.keys(importMapping).forEach((dbKey) => {
          const excelHeader = importMapping[dbKey];
          if (excelHeader && row[excelHeader] !== undefined) u[dbKey] = row[excelHeader];
        });
        return u;
      });

      const { duplicates, newUsers } = await checkUserDuplicatesAction(mappedUsers);

      if (duplicates && duplicates.length > 0) {
        setDuplicateQueue(duplicates);
        setNewUsersQueue(newUsers || []);
        setCurrentDuplicateIndex(0);
        setResolvedUpdates([]);
        ...
        const res = await finalizeUserImportAction(newUsers || [], []);
      ...
      console.error(err);
      toast.error(err.message || "Error importing users");
    } finally {
      setIsImporting(false);
  ...
  const handleNextDuplicate = async (action: "replace" | "keep" | "update" | "new") => {
    ...
    if (currentDuplicateIndex < duplicateQueue.length - 1) {
      ...
      setIsImporting(true);
      try {
        const res = await finalizeUserImportAction(newList, updatedList);
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-xl font-bold text-[#133255]">Import Users</h2>
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-xl p-12 flex flex-col items-center justify-center bg-gray-50/50">
                {isImporting ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-500 mb-2 text-center max-w-sm">
                      Our AI will automatically map your columns to the required fields. Supports .csv, .xlsx, .xls
                    </p>
                    <p className="text-xs text-amber-600 font-medium mb-6">Email column is required for user import</p>
              ...
              <div className="space-y-6">
                <div className="bg-blue-50 text-blue-800 p-4 rounded-lg text-sm flex gap-3">
                  ...
                  <div>
                    ...
                    <p>Review how your columns map to system fields. <span className="font-semibold">Email must be mapped</span> to import users.</p>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y">
                      {Object.keys(importMapping).map((dbKey) => (
                        <tr key={dbKey} className={!importMapping[dbKey] && dbKey === "email" ? "bg-red-50" : !importMapping[dbKey] ? "bg-orange-50/40" : ""}>
                          <td className="px-4 py-3 font-medium text-gray-700 capitalize">
                            {dbKey.replace(/([A-Z])/g, " $1").trim()}
                            {dbKey === "email" && <span className="text-red-500 ml-1">*</span>}
                          </td>
                ...
                <div className="flex justify-between gap-3 pt-4 border-t">
                  ...
                  <button onClick={confirmImport} disabled={isImporting} className="px-5 py-2.5 rounded-md bg-[#D8B15B] text-[#133255] font-bold hover:bg-[#e8c97a] transition-colors text-sm flex items-center gap-2">
                    ...
                    Check Duplicates & Import {importFileData.length} Users
                  </button>
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-[#0d162e]/50 backdrop-blur-sm z-50 flex items-center justify-center p-5">
          <div className="bg-white rounded-[20px] shadow-[0_30px_80px_rgba(0,0,0,0.3)] w-full max-w-[800px] flex flex-col max-h-[90vh]">
            <div className="px-6 py-5 border-b border-[#e4e8f0] flex justify-between items-center bg-[#f8fafc] rounded-t-[20px]">
              <div>
                <h3 className="font-serif text-[21px] font-bold text-gray-900">Resolve Duplicate Users</h3>
                <p className="text-sm text-[#5a6679] mt-1">User {currentDuplicateIndex + 1} of {duplicateQueue.length}</p>
              </div>
