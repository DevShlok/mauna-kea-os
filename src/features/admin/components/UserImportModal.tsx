...
export default function UserImportModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data: any = await mapUsersAction(headers.filter((h: string) => h), sampleData);
      
      if (!data || !data.mapping) {
        throw new Error("Failed to map users: AI returned empty response.");
      }
  ...
  const handleNextDuplicate = async (action: 'replace' | 'keep' | 'update' | 'new') => {
    ...
    if (action === 'replace') {
      ...
      // Import as a new user even if email exists (rare, but supported)
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                >
                  ...
                  Import {importFileData.length} Users
                </button>
              </div>
            </div>
          )}
          ...
          <div className="px-6 py-5 border-b border-[#e4e8f0] flex justify-between items-center bg-[#f8fafc] rounded-t-[20px]">
            <div>
              ...
              <p className="text-sm text-[#5a6679] mt-1">
                User {currentDuplicateIndex + 1} of {duplicateQueue.length}
              </p>
            </div>
