...
export default function PipelineImportModal({ isOpen, onClose, mandateId, mandateTitle, currentUser }: { isOpen: boolean, onClose: () => void, mandateId: number, mandateTitle: string, currentUser: { name: string } }) {
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      const res = await bulkInsertMandateCandidatesAction(mappedCandidates, mandateId, currentUser);

      if (res.failedCount && res.failedCount > 0) {
        toast.error(`Imported with some issues. Failed/Skipped ${res.failedCount} rows: ${res.failedRows?.join(', ')}`);
      } else {
