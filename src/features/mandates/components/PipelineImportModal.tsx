...
import { mapCandidatesAction, checkPipelineDuplicatesAction, finalizePipelineImportAction } from "@/actions/imports";
import { useRouter } from "next/navigation";

export default function PipelineImportModal({ isOpen, onClose, mandateId, mandateTitle, currentUser }: { isOpen: boolean, onClose: () => void, mandateId: number, mandateTitle: string, currentUser: { name: string } }) {
  ...
  const confirmImport = async () => {
    ...
    try {
      const mappedCandidates = importFileData.map(row => {
        ...
        Object.keys(importMapping).forEach(dbKey => {
          ...
          if (excelHeader && row[excelHeader] !== undefined) {
            c[dbKey] = row[excelHeader];
          }
        });
        return c;
      });

      const { duplicates, newCandidates } = await checkPipelineDuplicatesAction(mappedCandidates, mandateId);
      ...
      if (duplicates && duplicates.length > 0) {
        ...
        const res = await finalizePipelineImportAction(newCandidates || [], [], mandateId, currentUser);
  ...
  const handleNextDuplicate = async (action: 'replace' | 'keep' | 'update' | 'new') => {
    ...
    const newList = [...newCandidatesQueue];
    ...
    if (currentDuplicateIndex < duplicateQueue.length - 1) {
      ...
      setIsImporting(true);
      try {
        const res = await finalizePipelineImportAction(newList, updatedList, mandateId, currentUser);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-xl font-bold text-[#133255]">Import Candidates</h2>
            <p className="text-sm text-gray-500">
              For Mandate: <span className="font-semibold text-gray-700">{mandateTitle}</span>
            </p>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="border rounded-lg overflow-hidden">
                <table className="w-full text-left text-sm">
                  <thead className="bg-gray-50 border-b">
                    <tr>
                      <th className="px-4 py-3 font-medium text-gray-500 w-1/3">Candidate Field</th>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                >
                  ...
                  Import {importFileData.length} Candidates
                </button>
              </div>
            </div>
          )}
