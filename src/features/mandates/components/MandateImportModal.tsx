...
export default function MandateImportModal({
  isOpen, onClose, clientId, clientName, currentUser,
}: {
  isOpen: boolean;
  onClose: () => void;
  clientId?: string;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data: any = await mapMandatesAction(validHeaders, sampleData);
  ...
  const confirmImport = async () => {
    ...
    if (!clientName && !importMapping["company"]) {
      toast.error("Company column must be mapped for a global import.");
      return;
    }
    ...
    try {
      const mappedMandates = importFileData.map((row) => {
        ...
        Object.keys(importMapping).forEach((dbKey) => {
          ...
          if (excelHeader && row[excelHeader] !== undefined) m[dbKey] = row[excelHeader];
        });
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {clientName ? <>For client: <span className="font-semibold text-gray-600">{clientName}</span></> : "Global import — client records will be matched or created automatically"}
              </p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between gap-3 pt-4 border-t">
                  ...
                  <button onClick={confirmImport} disabled={isImporting} className="px-5 py-2.5 rounded-md bg-[#D8B15B] text-[#133255] font-bold hover:bg-[#e8c97a] transition-colors text-sm flex items-center gap-2">
                    ...
                    Check Duplicates & Import {importFileData.length} Mandates
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="bg-white rounded-[20px] shadow-[0_30px_80px_rgba(0,0,0,0.3)] w-full max-w-[800px] flex flex-col max-h-[90vh]">
            <div className="px-6 py-5 border-b border-[#e4e8f0] flex justify-between items-center bg-[#f8fafc] rounded-t-[20px]">
              <div>
                ...
                <p className="text-sm text-[#5a6679] mt-1">Mandate {currentDuplicateIndex + 1} of {duplicateQueue.length}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="border border-[#e4e8f0] rounded-[12px] p-5">
                  ...
                  <div className="space-y-4">
                    {Object.entries(duplicateQueue[currentDuplicateIndex]?.existingRecord || {}).map(([k, v]) => {
                      ...
                      return (<div key={k}><div className="text-[12px] text-[#8a93a3] capitalize">{k.replace(/([A-Z])/g, " $1").trim()}</div><div className="font-semibold text-gray-900 text-[14px]">{String(v)}</div></div>);
                    })}
                  </div>
                </div>
                <div className="border border-[#1d4ed8] rounded-[12px] p-5 bg-[#f8faff]">
                  <div className="text-[12px] font-bold text-[#1d4ed8] uppercase tracking-wider mb-4 border-b border-[#cfd6e4] pb-2 flex justify-between">
                    <span>Incoming (File)</span><span>Select to merge</span>
                  </div>
                  <div className="space-y-4">
