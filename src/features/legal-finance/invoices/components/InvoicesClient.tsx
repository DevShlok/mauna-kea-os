...
import { Plus, Search, FileText, Download, CheckCircle2, AlertTriangle, Clock, Eye, Ban, DollarSign } from "lucide-react";
...
export default function InvoicesClient({
  ...
  useEffect(() => {
    ...
    params.set("page", "1");
    router.push(`/dashboard/legal-finance/invoices?${params.toString()}`);
  }, [debouncedSearch, activeTab]);

  const getStatusBadge = (status: string, dueDateStr: string) => {
    const today = new Date().toISOString().split("T")[0];
    const isOverdue = status !== "Paid" && status !== "Cancelled" && dueDateStr < today;
    ...
    switch (status) {
      ...
      case "Cancelled":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-300 text-slate-700">
  ...
  const tabs = ["All", "Draft", "Shared", "Partially Paid", "Paid", "Overdue", "Cancelled", "Credit Note"];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Manage billing, tax invoices, credit notes, and collections
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200/80 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              type="text"
              placeholder="Search invoice no, PO..."
              value={search}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200/80 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                ...
                <th className="py-3.5 px-4 text-right">Fee (Excl GST)</th>
                  ...
                  <td colSpan={8} className="py-12 text-center text-slate-400">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {search || activeTab !== "All"
                        ...
                        : "Click 'Raise Invoice' to generate a tax invoice."}
                    </p>
                  </td>
                </tr>
              ) : (
                initialData.rows.map(({ invoice, clientName }) => (
                  <tr
                    key={invoice.id}
                  ...
                  >
                    <td className="py-3.5 px-4 font-semibold text-[#133255]">
                      <Link
                        href={`/dashboard/legal-finance/invoices/${invoice.id}`}
                    ...
                    <td className="py-3.5 px-4 text-right font-medium text-slate-700">
                      ₹{((invoice.feeBeforeTax || 0) / 100000).toFixed(2)} L
                    </td>
                    <td className="py-3.5 px-4 text-right font-bold text-[#133255]">
                      ₹{((invoice.totalAmount || 0) / 100000).toFixed(2)} L
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <Link
                        href={`/dashboard/legal-finance/invoices/${invoice.id}`}
                        className="inline-flex items-center gap-1 p-1.5 text-slate-500 hover:text-[#133255] hover:bg-slate-100 rounded-lg transition-colors"
                        title="View Details"
                      >
