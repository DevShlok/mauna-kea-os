...
import { mapFloatsAction, bulkInsertFloatsAction } from "@/actions/imports";
...
export default function FloatImportModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      const res = await bulkInsertFloatsAction(mappedfloats);
      ...
      if (res.failedCount && res.failedCount > 0) {
        ...
        toast.success("Successfully imported floats!");
      }
      setImportMapping(null);
      setImportFileData([]);
      onClose();
      ...
      console.error(err);
      toast.error("Error importing floats.");
    } finally {
      setIsImporting(false);
    }
  };
